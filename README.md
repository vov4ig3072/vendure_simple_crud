...
query getAllPosts{
  getAllPosts{
    items{
      title, content, authorName, id, createdAt
    }, 
  	totalItems
	}
}
...
mutation createPost($input: PostRewiewInput){
  createPost(input: $input){
    title,content,id
  }
}

mutation updatePost($updateInput: UpdatePostRewiewInput){
  updatePost(input: $updateInput){
    title,content,id
  }
}

mutation deletePost($deletedId: ID!){
  deletePost(postId: $deletedId){
  	id, title,content
  }
}
